...
for (let i=0; i<n; i++){
  ...
  .fromTo(cube.querySelectorAll('.face'), {
    // Màu bắt đầu - hồng nhạt đến đỏ đậm
    color:(j)=>'hsl('+(340)+', 85%,'+(100*[rots[3].a, rots[0].a, rots[1].a][j])+'%)'
  },{
    // Màu chuyển tiếp - hồng đậm
    color:(j)=>'hsl('+(350)+', 90%,'+(100*[rots[0].a, rots[1].a, rots[2].a][j])+'%)'
  }, 0)
  .to(cube.querySelectorAll('.face'), {
    // Màu kết thúc - đỏ tình yêu
    color:(j)=>'hsl('+(360)+', 95%,'+(100*[rots[1].a, rots[2].a, rots[3].a][j])+'%)'
  }, 1)
  .progress(i/n)
}
